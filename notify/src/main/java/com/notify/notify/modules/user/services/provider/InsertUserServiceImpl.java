...
public class InsertUserServiceImpl implements InsertUserService {
    ...
    @Override
    @ResultTransaction
    @ObservedMetric("insert.user.service")
    public Result<UserEntity> execute(UserEntity user) {
        try {
            repository.insert(user);

            log.info("Forced insert of user id={}, version={}", user.getId(), user.getVersion());

            return Result.success(user, HttpStatus.CREATED);
            ...
            if (message.toLowerCase().contains("uk_users_email")) {
                return Result.failure("Email already exists", HttpStatus.FORBIDDEN);
            }

            if (message.toLowerCase().contains("uk_users_name")) {
                return Result.failure("Name already exists", HttpStatus.FORBIDDEN);
            }
